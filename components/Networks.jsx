  ...
  return (
    <section id="generator" className="py-14">
      <div className="container z-10">
        <div className="grid xl:grid-clos-3 md:grid-cols-2 grid-cols-1 gap-6 py-20">
          {networks?.map((network, index) => (
            <div key={index +1 } onClick={() => selectNetwork(network)}>
              <div
                className={`group p-8 rounded-xl bg-default-950/40 transition-all duration-500
                 hover:-translate-y-2 hover:bg-primary/40 ${
                  activeNetwork == network.name ? "bg-primary/40" : ""
                 }`}
                 >
                  <div className="h-14 w-14 flex items-center justify-center
                   rounded-lg bg-primary 
                   text-primary group-hover:bg-white/20 group-hover:text-white">
                ...
                <p className="text-base font-normal text-default-300 mt-4">
                  ...
                  <strong>{network.name}</strong>, you can able to
                  find and get
                  access of the details for getting the Pool
                  address and 
                  liquidity 
                </p>
              </div>
